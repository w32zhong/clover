...
def ea_forward(input_ids, model, tokenizer, tree_choices, logits_processor=None, max_steps=2048, max_length=-1):
    ...
    outputs, _ = model.base_model(input_ids, past_key_values=past_key_values, use_cache=True)
    new_token = 0
    ...
    for idx in range(max_steps):
        ...
        outputs, _ = model.base_model(input_id, use_cache=True, past_key_values=past_key_values)

        if hasattr(model, 'timer'): model.timer._hist['bonus tokens'].append(input_id.shape[-1] - 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-id", type=str, default="ess-vicuna-7b-fp16-baseline")
    parser.add_argument(
        "--bench-name",
        type=str,
        default="mt_bench",
    ...
    args.model_id = args.model_id + "-spe-8b-temperature-" + str(args.temperature)
    ...
    question_file = f"{parent_dir}/data/{args.bench_name}/question_spe.jsonl"
    if args.answer_file:
